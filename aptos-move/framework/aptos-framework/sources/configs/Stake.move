...
module AptosFramework::Stake {
    ...
    const REWARDS_RATE_PERCENTAGE: u64 = 1;
    const MINIMUM_LOCK_UP_SECS: u64 = 24 * 60 * 60; // 1 day.
    const MAXIMUM_LOCK_UP_SECS: u64 = 7 * 24 * 60 * 60; // 7 days.
    ...
    public(friend) fun update_performance_scores(missed_votes: vector<u64>) acquires ValidatorPerformance {
    ...
    /// power.
    public(friend) fun on_new_epoch() acquires StakePool, TestCoinCapabilities, ValidatorConfig, ValidatorPerformance, ValidatorSet {
        let validator_set = borrow_global_mut<ValidatorSet>(@CoreResources);
        let validator_perf = borrow_global_mut<ValidatorPerformance>(@CoreResources);
        ...
        while (i < len) {
            ...
            update_stake_pool(validator_perf, addr);
            i = i + 1;
        };
        ...
        while (i < len) {
            ...
            let stake_pool = borrow_global_mut<StakePool>(new_validator_info.addr);
            if (new_validator_info.voting_power >= validator_set.minimum_stake &&
                ...
                stake_pool.validator_index = Vector::length(&active_validators);
                Vector::push_back(&mut active_validators, new_validator_info);
                Vector::push_back(&mut validator_perf.missed_votes, 0);
            } else {
                // Reset validator index of validators that are no longer active.
                stake_pool.validator_index = 0;
            };
            i = i + 1;
        };
    ...
    fun update_stake_pool(
        ...
        pool_address: address,
    ) acquires StakePool, TestCoinCapabilities {
        let stake_pool = borrow_global_mut<StakePool>(pool_address);
        let num_missed_votes = *Vector::borrow(&validator_perf.missed_votes, stake_pool.validator_index);
        ...
        distribute_reward(&mut stake_pool.active, num_blocks, num_successful_votes, remaining_lockup_time);
        distribute_reward(&mut stake_pool.pending_inactive, num_blocks, num_successful_votes, remaining_lockup_time);
    ...
    fun distribute_reward(
        ...
        let base_rewards = Coin::value<TestCoin>(stake) * REWARDS_RATE_PERCENTAGE / 100;
        let rewards_denominator = num_blocks * MAXIMUM_LOCK_UP_SECS;
    ...
    fun validate_lockup_time(locked_until_secs: u64) {
        let current_time = Timestamp::now_seconds();
        assert!(current_time + MINIMUM_LOCK_UP_SECS <= locked_until_secs, Errors::invalid_argument(ELOCK_TIME_TOO_SHORT));
        assert!(locked_until_secs <= current_time + MAXIMUM_LOCK_UP_SECS, Errors::invalid_argument(ELOCK_TIME_TOO_LONG));
    }
    ...
    public(script) fun test_basic_staking(
        ...
        validator: signer,
    ) acquires OwnerCapability, StakePool, TestCoinCapabilities, ValidatorConfig, ValidatorPerformance, ValidatorSet {
        use AptosFramework::TestCoin;
        ...
        increase_lockup(&validator, MAXIMUM_LOCK_UP_SECS);
        assert!(Coin::balance<TestCoin>(validator_address) == 900, 0);
        assert_stake_pool(validator_address, 100, 0, 0, 0, 0);
        ...
        assert_stake_pool(validator_address, 101, 0, 0, 100, 0);
        on_new_epoch();
        ...
        assert_stake_pool(validator_address, 101, 0, 0, 0, 0);
    }
    ...
    public(script) fun test_delegated_staking_with_owner_cap(
        ...
        assert_stake_pool(pool_address, 100, 0, 0, 0, 0);
        ...
        assert_stake_pool(pool_address, 0, 0, 0, 100, 0);
        on_new_epoch();
        ...
        assert_stake_pool(pool_address, 0, 0, 0, 0, 0);
    ...
    public(script) fun test_validator_join_leave(
        ...
        increase_lockup(&validator_1, MAXIMUM_LOCK_UP_SECS);
        add_stake(&validator_2, 100);
        increase_lockup(&validator_2, MAXIMUM_LOCK_UP_SECS);
        add_stake(&validator_3, 100);
        increase_lockup(&validator_3, MAXIMUM_LOCK_UP_SECS);
        assert_stake_pool(validator_1_address, 100, 0, 0, 0, 0);
        assert_stake_pool(validator_2_address, 100, 0, 0, 0, 0);
        assert_stake_pool(validator_3_address, 100, 0, 0, 0, 0);
        ...
        // Validator indices are reversed due to the way pending validators are appended into the active set.
        assert_stake_pool(validator_1_address, 100, 0, 0, 0, 1);
        assert_stake_pool(validator_2_address, 100, 0, 0, 0, 0);
        ...
        assert_stake_pool(validator_1_address, 101, 0, 0, 0, 0);
        assert!(!is_current_validator(validator_2_address), 4);
        assert_stake_pool(validator_2_address, 101, 0, 0, 0, 0);
        assert!(is_current_validator(validator_3_address), 5);
        assert_stake_pool(validator_3_address, 100, 0, 0, 0, 1);
    ...
    public(script) fun register_mint_stake(
        ...
        assert_stake_pool(Signer::address_of(account), 100, 0, 0, 0, 0);
    }
    ...
    fun assert_stake_pool(
        ...
        assert!(stake_pool.validator_index == validator_index, 4);
    }
}
